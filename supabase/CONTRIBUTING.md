...
| Kind | Where | When |
| --- | --- | --- |
| Schema / policy changes | `migrations/` | New timestamped SQL file; do not rewrite already-applied migrations on shared branches |
| Static SQL fixtures | `seeds/` | Data loaded on `supabase db reset` |
...
- Keep seed UUIDs / emails **stable** once referenced by backend tests or `e2e/tests/helpers/auth.ts`.
- Define users once in `python_seeds/data/` and import them elsewhere — avoid duplicated credentials.
